...
TCP/IPは、インターネットで実際に使用されているプロトコル群である。
OSI参照モデルとは異なり、[4つの層](https://www.cisco.com/c/ja_jp/support/docs/ip/routing-information-protocol-rip/13769-5.html)に分けられている。
TCP/IPは、UNIXがサポートしたことを背景として、現在最も普及しているモデルである。

通信データは送信時に、カプセル化を行う。
各層で、データを送信するための情報をヘッダとして付け加える。
例えば、ネットワーク層では宛先IPアドレスが追加される。トランスポート層では、通信の信頼性を確保するための制御情報が追加される。
データリンク層では、MACアドレスが追加される。
受信側では、非カプセル化を行う。
各層で、ヘッダが取り除かれ、最終的にアプリケーションへデータが渡される。
...
## デフォルトゲートウェイ
デフォルトゲートウェイは、ネットワークの出入り口に相当する機器(ルーター)のIPアドレスである。
...
## 参考
1. mastering TCP/IP　Introductory Level 
2. 一週間でCCNAの基礎が学べる本
3. [インフラエンジニアを目指すための「ネットワーク（CCNA）学習」](https://www.youtube.com/playlist?list=PLXXalsdlzX-J7AUuw2zSHit5nRUZ3eDdz)
